...
def decide_dset_type(dd) -> str:
    if ("chair" in dd or "drums" in dd or "ficus" in dd or "hotdog" in dd
        ...
        return "synthetic"
          ...
          or "room" in dd or "trex" in dd or "and-bot" in dd or "yoda_origin" in dd or "t_balloon_statue"):
        return "llff"
    else:
...
def init_tr_data(data_downsample: float, data_dirs: Sequence[str], **kwargs):
    ...
    if dset_type == "synthetic":
        ...
        dset = SyntheticNerfDataset(
            ...
            max_frames=max_tr_frames, batch_size=batch_size, is_robust_loss_enabled=is_robust_loss_enabled, patch_size=patch_size, log_dir=log_dir)
    elif dset_type == "llff":
